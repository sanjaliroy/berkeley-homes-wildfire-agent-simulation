...
def judge_intervention_v2(
    ...
    """
    system = (
        "You evaluate how faithfully a simulated agent represents how a real person "
        "with this background would respond to wildfire mitigation interventions. "
        "Read the seed narrative carefully — judge against who THIS person is, not "
        "a generic cooperative resident.\n\n"
        "Watch for LLM social desirability bias: simulated agents tend to comply "
        "more readily and sound more reasonable than real people would. Treat "
        "over-cooperation as a fidelity failure.\n\n"
        "Score 5-6 for plausible but generic responses. Score 7+ only when you can "
        "cite specific evidence. Reserve 9-10 for clearly distinctive responses."
    )
    user = (
        f"SEED NARRATIVE:\n{seed_narrative}\n\n"
        f"INTERVENTION:\n{intervention}\n\n"
        f"DECISION:\n{decision}\n\n"
        f"REASONING:\n{reasoning}\n\n"
        "Write a 1-2 sentence note citing specific evidence, then give an integer "
        "score 1-10. Do not score above 6 without specific evidence.\n\n"
        "1. BEHAVIORAL PLAUSIBILITY — Would this person actually respond this way "
        "given their role, constraints, and worldview?\n"
        "   1-2 = Contradicts their actual situation (role, finances, constraints)\n"
        "   3-4 = Generic — any reasonable person in a similar situation would respond this way\n"
        "   5-6 = Fits without contradictions but their specific frictions don't shape the decision\n"
        "   7-8 = Their specific constraints or worldview visibly drive the response\n"
        "   9-10 = Reflects a tension unique to this person a generic stand-in couldn't produce\n"
        "   Failures: social desirability bias, trait neutralisation, trajectory discontinuity "
        "(ignoring prior actions or commitments from earlier in the simulation)\n\n"
        "2. PERSONA CONSISTENCY — Does this sound like this specific person or like anyone?\n"
        "   1-2 = Contradicts the seed in tone, values, or decision\n"
        "   3-4 = Fits the general persona type but nothing specific to this person surfaces\n"
        "   5-6 = No inconsistencies but their distinctive voice or opinions don't come through\n"
        "   7-8 = Their characteristic framing or attitude is clearly present\n"
        "   9-10 = Could only have come from this person\n"
        "   Failures: trait neutralisation, contradicts a key seed trait\n\n"
        "3. INTERVENTION RESPONSIVENESS — Did the agent engage with what was actually said?\n"
        "   1-2 = Ignored the content — generic response about their situation\n"
        "   3-4 = Acknowledged the event type but not the specific details\n"
        "   5-6 = Engaged with the main thrust but missed key specifics\n"
        "   7-8 = Engaged with specific details (deadlines, amounts, requirements)\n"
        "   9-10 = Integrated specifics with prior context showing genuine processing\n\n"
        "Respond in this exact JSON format:\n"
        "{\n"
        '  "note_plausibility": "<evidence>",\n'
        '  "behavioral_plausibility": <integer 1-10>,\n'
        '  "note_consistency": "<evidence>",\n'
        '  "persona_consistency": <integer 1-10>,\n'
        '  "note_responsiveness": "<evidence>",\n'
...
def judge_full_simulation_v2(
    ...
    system = (
        "You evaluate how faithfully a simulated agent represents how a real person "
        "with this background would behave across a full wildfire mitigation simulation. "
        "Read the seed narrative carefully — judge against who THIS person is, not "
        "a generic cooperative resident.\n\n"
        "Give one holistic score per criterion — not an average of per-event scores. "
        "Assess trajectory-level patterns: consistency across events, whether the "
        "agent remembers and builds on prior decisions, and cumulative plausibility.\n\n"
        "Watch for LLM social desirability bias: consistent over-cooperation across "
        "events is itself a fidelity failure. Score 5-6 for plausible but generic "
        "trajectories. Score 7+ only when you can cite specific evidence. Reserve "
        "9-10 for clearly distinctive trajectories."
    )
    user = (
        f"SEED NARRATIVE:\n{seed_narrative}\n\n"
        f"FULL TRAJECTORY ({len(all_decisions)} events):\n{trajectory}\n\n"
        "Write a 1-2 sentence note citing specific evidence, then give an integer "
        "score 1-10. Do not score above 6 without specific evidence.\n\n"
        "1. BEHAVIORAL PLAUSIBILITY — Would this person actually behave this way "
        "across the simulation given their role, constraints, and worldview?\n"
        "   1-2 = Contradicts their actual situation across multiple events\n"
        "   3-4 = Generic — any reasonable person in a similar situation would behave this way\n"
        "   5-6 = Fits without contradictions but their specific frictions don't shape responses\n"
        "   7-8 = Their constraints or worldview consistently drive specific decisions\n"
        "   9-10 = Trajectory reflects tensions unique to this person a stand-in couldn't produce\n"
        "   Failures: social desirability bias, trait neutralisation, trajectory discontinuity "
        "(agent doesn't reference completed actions or commitments from earlier events)\n\n"
        "2. PERSONA CONSISTENCY — Does this trajectory sound like this specific person throughout?\n"
        "   1-2 = Contradicts the seed personality across multiple events\n"
        "   3-4 = Fits the general persona type but nothing specific to this person surfaces\n"
        "   5-6 = No inconsistencies but their distinctive voice or opinions don't come through\n"
        "   7-8 = Their characteristic framing or attitude is consistently present\n"
        "   9-10 = Could only have come from this person throughout\n"
        "   Failures: trait neutralisation, agent contradicts their own earlier decisions\n\n"
        "3. INTERVENTION RESPONSIVENESS — Did the agent engage with each intervention's "
        "specific content across the trajectory?\n"
        "   1-2 = Rarely engaged with specifics — mostly generic situation responses\n"
        "   3-4 = Acknowledged event types but not details\n"
        "   5-6 = Engaged with main thrusts but typically missed key specifics\n"
        "   7-8 = Consistently engaged with specific details across events\n"
        "   9-10 = Consistently integrated specifics with prior context across the simulation\n\n"
        "Respond in this exact JSON format:\n"
        "{\n"
        '  "note_plausibility": "<evidence>",\n'
        '  "behavioral_plausibility": <integer 1-10>,\n'
