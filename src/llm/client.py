...
def judge_intervention_v2(
    ...
    user = (
        ...
        "this person's background would predict), trait neutralisation (agent with "
        "a distinctive worldview responds like an average person), and trajectory "
        "discontinuity (agent ignores things they have already done or decided — "
        "e.g. treating mitigation work as unstarted when they previously completed it, "
        "or not referencing a prior commitment they made)\n\n"
...
def judge_full_simulation_v2(
    ...
    user = (
        ...
        "events), trait neutralisation (distinctive persona consistently responds "
        "like an average person), and trajectory discontinuity (agent treats each "
        "event as if the prior simulation history never happened — for example, "
        "not referencing completed mitigation work, forgotten commitments, or "
        "actions taken in earlier events)\n\n"
